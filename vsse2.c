                ...
                if (simd_prefix == 1) { //66
                    if (leading_opcode == 1) {//0F
                        ...
                        if (is_saved_state64(regs)) {
                            M64 r64res;
                            r64res.u64 = xmmres.u64[0];
                            _load_m64(num_dst, &r64res, regs);
                        } else {
                            M32 r32res;
                            r32res.u32 = xmmres.u32[0];
                            _load_m32(num_dst, &r32res, regs);
                        }
                    }
                }
                break;
            ...
            case 0x2A:
                ...
                if (simd_prefix == 3) { //F2
                    if (leading_opcode == 1) {//0F
                        if (mod == 3) {
                            M64 r64src;
                            _store_m64(num_src, &r64src, regs);
